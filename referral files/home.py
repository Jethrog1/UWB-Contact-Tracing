...
def notification_handler(sender, data):
    parse_and_store(data.decode('utf-8').strip())
...
def draw_map_scene(painter, w, h, scale, origin,
    ...
    # reference calculator dot — outside anchor loop
    if ref_dot:
        rx, ry = ref_dot
        sp = ts(rx, ry)
        arm = 14
    ...
    # inter-tag distance lines (drawn before dots so dots appear on top)
...
# ── floating ghost label that follows cursor during tab drag ──────────────────
...
class RightPanel(QFrame):
    def __init__(self):
        ...
        bi.addWidget(section_label("BLE CONNECTIVITY")); bi.addWidget(h_line())
        self._ble_labels={}
        for tid in ["T0","T1","T2"]:
        ...
        # ── MAP PARAMETERS ────────────────────────────────
        mp_box = section_box();
        mi = QVBoxLayout(mp_box);
        mi.setContentsMargins(10, 8, 10, 8);
        mi.setSpacing(6)
        mi.addWidget(section_label("MAP PARAMETERS"));
        mi.addWidget(h_line())
        self.anchor_edits = {}
        for aid in ["A0", "A1", "A2", "A3"]:
            row = QHBoxLayout();
            row.setSpacing(6)
            dot = QLabel("●");
            dot.setStyleSheet(f"color:{ANCHOR_COLORS[aid]};font-size:13px;");
            dot.setFixedWidth(16)
            id_lbl = QLabel(aid);
            id_lbl.setStyleSheet(f"color:{ANCHOR_COLORS[aid]};font-weight:600;font-size:11px;");
            id_lbl.setFixedWidth(26)
            edit = QLineEdit("0.0, 0.0");
            ...
            self.anchor_edits[aid] = edit;
            mi.addLayout(row)
        mi.addSpacing(4)
        ll = QLabel("LINES");
        ll.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:10px;letter-spacing:1px;")
        mi.addWidget(ll)
        self.lines_container = QVBoxLayout();
        self.lines_container.setSpacing(3)
        mi.addLayout(self.lines_container)
        self.add_line_btn = QPushButton("+ Add Line");
        self.add_line_btn.setObjectName("paramBtn")
        self.add_line_btn.clicked.connect(self._show_picker);
        mi.addWidget(self.add_line_btn)
        self._picker = QWidget();
        self._picker.setStyleSheet(f"background:{SURFACE};border-radius:4px;")
        pk = QVBoxLayout(self._picker);
        pk.setContentsMargins(6, 6, 6, 6);
        pk.setSpacing(4)
        ...
        for aid in ["A0", "A1", "A2", "A3"]:
            btn = QPushButton(aid);
            btn.setObjectName("anchorSelectBtn")
            btn.setProperty("selected", "false");
            btn.setFixedHeight(28)
            btn.clicked.connect(lambda c, a=aid: self._picker_click(a))
            btn_row.addWidget(btn);
            self._picker_btns[aid] = btn
        pk.addLayout(btn_row)
        cpk = QPushButton("Esc / Cancel");
        cpk.setObjectName("hideBtn");
        cpk.clicked.connect(self._cancel_picker)
        pk.addWidget(cpk);
        self._picker.hide();
        mi.addWidget(self._picker)
        mi.addSpacing(6)
        mi.addWidget(h_line())
        height_row = QHBoxLayout();
        height_row.setSpacing(6)
        ht_icon = QLabel("↕");
        ht_icon.setStyleSheet(f"color:{ACCENT};font-size:13px;font-weight:700;");
        ht_icon.setFixedWidth(16)
        ht_lbl = QLabel("Anchor to Tag height (ft):");
        ht_lbl.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:10px;font-weight:600;")
        self._height_offset_edit = QLineEdit("0.0");
        self._height_offset_edit.setFixedHeight(24)
        ...
        height_row.addWidget(ht_icon);
        height_row.addWidget(ht_lbl);
        height_row.addWidget(self._height_offset_edit, stretch=1)
        ...
        for tid in ["T0","T1","T2"]:
            ...
            sep = QFrame(); sep.setObjectName("hLine"); sep.setFrameShape(QFrame.Shape.HLine)
            tl.addWidget(sep)
            dist_edits = {}
            for aid in ["A0","A1","A2","A3"]:
        ...
        for aid in ["A0","A1","A2","A3"]:
            ...
            # default: Polynomial selected so show Deg, hide MA
        ...
        # ── Captured Points Log ──────────────────────────────────────────────
        pts_box = section_box()
        pi = QVBoxLayout(pts_box);
        pi.setContentsMargins(10, 8, 10, 8);
        pi.setSpacing(6)
        pi.addWidget(section_label("CAPTURED POINTS"))
        pi.addWidget(h_line())
        ...
        clear_btn = QPushButton("Clear Captured Data")
        clear_btn.setObjectName("paramBtn")
        clear_btn.clicked.connect(self._clear_cal_data)
        pi.addWidget(clear_btn)
        ...
        # FIX 2: only one initialization of _last_raws/_last_cals
    ...
    # FIX 3: _calc_reference was missing — restored
        ...
        for aid in ["A0","A1","A2","A3"]:
            ...
            if self._auto_cal_checks[aid].isChecked():
                ...
                self._eq_text_edits[aid].setStyleSheet(
                    f"background:{DARK_BG};color:{col};border:1px solid {BORDER};"
                    f"border-radius:3px;padding:2px 6px;font-size:9px;font-family:Consolas;")
                # update the dropdown to reflect the winning mode — block signal to avoid recursion
                ...
                # update sub-option visibility only (no recalc)
    ...
    def _capture_tick(self):
        ...
        for aid in self._capture_true:
            ...
            # live burst scatter: plot raw samples at their true dist y-value
    ...
    def _animate_capture_fusion(self, frame):
        ...
        if frame <= total:
            ...
            # finalize — clear burst dots, commit mean point
            for aid, samples in self._capture_buf.items():
                if samples:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._ble_timer=QTimer(); self._ble_timer.timeout.connect(self._poll_ble); self._ble_timer.start(500)
        ...
        if HAS_BLEAK:
            ...
            self._demo_state=0
            self._demo_timer=QTimer(); self._demo_timer.timeout.connect(self._demo_tick); self._demo_timer.start(2000)
    ...
    def _update_cal_graph_live(self):
        ...
        ax = self.bottom_panel.ax
        # update live reading diamonds — current raw vs calibrated per anchor
        # only redraws if tag is connected and sending data
        needs_redraw = False
        for aid in ["A0", "A1", "A2", "A3"]:
            raw = tag_data.get(tid, {}).get(aid, -1)
            if raw > 0:
    ...
    def _demo_tick(self):
        states=[
            {"T0":"Connected","T1":"Connecting...","T2":"Disconnected"},
            {"T0":"Connected","T1":"Connected",    "T2":"Connecting..."},
            {"T0":"Connected","T1":"Connected",    "T2":"Connected"},
        ]
        s=states[self._demo_state%3]
        for tid,st in s.items():
            prev=_prev_tag_status[tid]
            if st!=prev:
                self.right_panel.notify_popups_ble(tid,st)
                _prev_tag_status[tid]=st
        tag_status.update(s); self.right_panel.update_ble_status(tag_status)

        # inject simulated UWB distances for any connected tag so capture and RTLS work in demo mode
        for tid, st in s.items():
            if st == "Connected":
                for aid in ["A0","A1","A2","A3"]:
                    # base distance ~7ft with small noise, simulating a tag near center of default 10x10 grid
                    tag_data[tid][aid] = 7.0 + random.gauss(0, 0.15)
            else:
                tag_data[tid] = {"A0":-1,"A1":-1,"A2":-1,"A3":-1}
    ...
    # FIX 4: removed dead tid = ref_tabs.currentIndex() line; FIX 5: call update_data
    def _rtls_loop(self):
        canvas = self.main_panel.canvas
        rp = self.right_panel
        anchor_positions = {aid: canvas.anchors[aid] for aid in canvas.anchors}

        # read height offset once before the loop — default 0 if blank/invalid
        try:
        ...
        for t_id in ["T0","T1","T2"]:
            ...
            # collect raw distances and immediately correct for height offset
            raw_d = {}
            for aid in ["A0","A1","A2","A3"]:
            ...
            # raw position using height-corrected floor distances
            rx, ry = calc_pos(anchor_positions, raw_d)
            canvas.tag_raw[t_id] = (rx, ry) if rx is not None else None

            # calibrated distances — apply cal_funcs to already height-corrected values
            cal_d = {}
