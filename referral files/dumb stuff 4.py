...
from PyQt6.QtCore import Qt, QSize, QPointF, QRectF, QEvent, QTimer, QPoint, QRect
from PyQt6.QtGui import (
    QColor, QPalette, QPainter, QPen, QBrush, QFont, QWheelEvent,
    QMouseEvent, QKeyEvent, QFontMetrics, QCursor, QPixmap, QImage
)
import numpy as np
import warnings
try:
...
WALK_TICK_MS        = 30
PERSON_SIZE         = 56          # px — sprite render size on canvas
MIN_MOVE_DIST_PX    = 60
MAX_MOVE_DIST_PX    = 260
MIN_TOP_SPEED       = 1.0
MAX_TOP_SPEED       = 3.8
MIN_ACCEL           = 0.03
MAX_ACCEL           = 0.10
STOP_PAUSE_MIN      = 15
STOP_PAUSE_MAX      = 50
FRAME_BASE_TIME     = 0.38
STOP_ANIM_SPEED     = 0.12
MIN_SPEED_FACING    = 0.15
DIR_SMOOTHING       = 0.18
DIR_HISTORY         = 8
...
def _smooth_angle(cur, tgt, amt):
    return cur + _wrap_angle(tgt-cur)*amt

def _mean_angle(vecs):
    if not vecs: return None
    sx = sum(d[0] for d in vecs); sy = sum(d[1] for d in vecs)
    if abs(sx)<1e-9 and abs(sy)<1e-9: return None
    return math.atan2(sy, sx)

def _speed_to_frame_time(spd):
    if spd <= STOP_ANIM_SPEED: return None
    if spd < 1.0: return FRAME_BASE_TIME
    if spd < 2.0: return FRAME_BASE_TIME*0.85
    if spd < 3.0: return FRAME_BASE_TIME*0.70
    if spd < 4.0: return FRAME_BASE_TIME*0.55
    return FRAME_BASE_TIME*0.40

def _load_walk_frames():
    """Load or generate fallback walking sprites."""
    def fallback(name):
        px = QPixmap(72, 72); px.fill(Qt.GlobalColor.transparent)
        p = QPainter(px); p.setRenderHint(QPainter.RenderHint.Antialiasing)
        p.setPen(QPen(Qt.GlobalColor.white, 3)); p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawEllipse(24,4,18,18); p.drawLine(33,22,33,48); p.drawLine(20,31,46,31)
        if name=="left":   p.drawLine(33,48,22,64); p.drawLine(33,48,45,57)
        elif name=="right": p.drawLine(33,48,26,57); p.drawLine(33,48,48,64)
        else:               p.drawLine(33,48,25,62); p.drawLine(33,48,41,62)
        p.end(); return px
    frames = {}
    for name, path in [("stand",_IMG_STAND),("left",_IMG_LEFT),("right",_IMG_RIGHT)]:
        pm = QPixmap(path) if path else QPixmap()
        frames[name] = pm if not pm.isNull() else fallback(name)
    return frames

# Lazily loaded at first use so Qt app exists
_WALK_FRAMES_CACHE = None
def get_walk_frames():
    global _WALK_FRAMES_CACHE
    if _WALK_FRAMES_CACHE is None:
        _WALK_FRAMES_CACHE = _load_walk_frames()
    return _WALK_FRAMES_CACHE

# ── FAKE TAG (world-space: x,y in feet, bounded inside anchor polygon) ────────
class FakeTag:
    """Simulated tag that moves in world-space (feet), bounded within the anchor polygon."""
    def __init__(self, tag_id: str, wx: float, wy: float):
        self.tag_id  = tag_id
        self.wx = wx; self.wy = wy          # world-space position (feet)
        self.prev_wx = wx; self.prev_wy = wy
        self.vx = 0.0; self.vy = 0.0
        self.speed = 0.0; self.angle = 0.0
        self.target_angle = 0.0; self.target_distance = 0.0
        self.distance_traveled = 0.0
        self.top_speed = 0.0; self.accel = 0.0; self.pause_ticks = 0
        self.is_walking = False; self.walk_phase = 0; self.anim_elapsed = 0.0
        self.direction_samples: list = []
        self._pick_new_motion()

    def _pick_new_motion(self):
        self.target_angle      = random.uniform(0, math.tau)
        # target distance in feet (0.3 – 2.5 ft per burst feels natural)
        self.target_distance   = random.uniform(0.3, 2.5)
        self.distance_traveled = 0.0
        # speed in ft/tick  (~0.02–0.08 ft per 30ms ≈ 0.7–2.7 ft/s)
        self.top_speed   = random.uniform(0.018, 0.075)
        self.accel       = random.uniform(0.002, 0.008)
        self.pause_ticks = random.randint(STOP_PAUSE_MIN, STOP_PAUSE_MAX)

    def _update_animation(self):
        ft = _speed_to_frame_time(max(self.speed * 20, STOP_ANIM_SPEED + 1e-6))
        if self.speed * 20 <= STOP_ANIM_SPEED:
            if self.anim_elapsed > 0:
                self.anim_elapsed += WALK_TICK_MS / 1000.0
                if ft and self.anim_elapsed >= ft:
                    self.is_walking = False; self.walk_phase = 0; self.anim_elapsed = 0.0
            else:
                self.is_walking = False; self.walk_phase = 0
            return
        self.is_walking = True
        self.anim_elapsed += WALK_TICK_MS / 1000.0
        if ft:
            while self.anim_elapsed >= ft:
                self.anim_elapsed -= ft
                self.walk_phase = 1 - self.walk_phase

    def current_frame_name(self):
        if not self.is_walking: return "stand"
        return "left" if self.walk_phase == 0 else "right"

    def update(self, world_poly):
        """world_poly: list of (wx,wy) anchor corners in world-space (feet)."""
        self.prev_wx = self.wx; self.prev_wy = self.wy
        if self.pause_ticks > 0:
            self.pause_ticks -= 1
            self.speed = max(0.0, self.speed - self.accel * 1.4)
        else:
            remaining = self.target_distance - self.distance_traveled
            brake_d   = self.speed ** 2 / (2.0 * max(self.accel, 1e-6))
            if remaining <= brake_d: self.speed = max(0.0, self.speed - self.accel)
            else:                    self.speed = min(self.top_speed, self.speed + self.accel)

        dvx = math.cos(self.target_angle) * self.speed
        dvy = math.sin(self.target_angle) * self.speed
        nwx = self.wx + dvx
        nwy = self.wy + dvy
        self.distance_traveled += math.hypot(dvx, dvy)

        if world_poly and len(world_poly) >= 3:
            if not _point_in_poly(nwx, nwy, world_poly):
                # try x only
                if not _point_in_poly(self.wx + dvx, self.wy, world_poly):
                    self.target_angle = math.pi - self.target_angle; dvx = -dvx
                # try y only
                if not _point_in_poly(self.wx, self.wy + dvy, world_poly):
                    self.target_angle = -self.target_angle; dvy = -dvy
                nwx = self.wx + dvx; nwy = self.wy + dvy
                if not _point_in_poly(nwx, nwy, world_poly):
                    # nudge toward centroid
                    cx = sum(p[0] for p in world_poly) / len(world_poly)
                    cy = sum(p[1] for p in world_poly) / len(world_poly)
                    self.target_angle = math.atan2(cy - self.wy, cx - self.wx)
                    nwx, nwy = self.wx, self.wy

        self.wx = nwx; self.wy = nwy

        dx = self.wx - self.prev_wx; dy = self.wy - self.prev_wy
        spd = math.hypot(dx, dy)
        if spd > 1e-5:
            self.direction_samples.append((dx, dy))
            if len(self.direction_samples) > DIR_HISTORY: self.direction_samples.pop(0)
            ma = _mean_angle(self.direction_samples)
            if ma is not None: self.angle = _smooth_angle(self.angle, ma, DIR_SMOOTHING)

        if self.pause_ticks == 0 and self.speed <= 1e-4 and self.distance_traveled >= self.target_distance:
            self._pick_new_motion()
        self._update_animation()

def _point_in_poly(x, y, pts):
    """Ray-casting point-in-polygon test. pts: list of (x,y) in any consistent space."""
...
def _convex_hull_world(anchor_dict):
    """Return convex hull of anchor positions in world-space (feet).
    Returns list of (wx,wy)."""
    pts = [(v[0], v[1]) for v in anchor_dict.values()]
...
def _poly_area(pts):
    """Shoelace formula — area of polygon with vertices pts = [(x,y),...]."""
    n = len(pts)
    if n < 3: return 1.0
    a = 0.0
    for i in range(n):
...
def _build_heatmap_lut():
    ...
    for i in range(256):
        v = i/255.0
        if v < 0.14:
            t=v/0.14;           r=0;           g=int(120*t);             b=255
        elif v < 0.28:
            t=(v-0.14)/0.14;    r=0;           g=int(120+135*t);         b=255
        elif v < 0.42:
            t=(v-0.28)/0.14;    r=0;           g=255;                    b=int(255*(1-t))
        elif v < 0.58:
            t=(v-0.42)/0.16;    r=int(255*t);  g=255;                    b=0
        elif v < 0.74:
            t=(v-0.58)/0.16;    r=255;         g=int(255-90*t);          b=0
        elif v < 0.88:
            t=(v-0.74)/0.14;    r=255;         g=int(165*(1-t));         b=0
        else:
            t=(v-0.88)/0.12;    r=int(255-75*t); g=0;                   b=0
        lut[i] = [r, g, b]
    return lut

HEATMAP_LUT = _build_heatmap_lut()
...
def draw_map_scene(painter, w, h, scale, origin,
                   ...
                   ref_dot=None, heatmap_active=False):
    p = painter
    if not heatmap_active:
...
class MapCanvas(QWidget):
    ...
    def __init__(self, right_panel_ref=None):
        ...
        # ── Themes / heatmap state ────────────────────────────────────────────
        self.heatmap_enabled   = False
        self.walk_anim_enabled = False
        self._hm_base          = 0.06
        self._hm_decay         = 0.993
        self._hm_exposure_ft   = 8.0        # ft radius that triggers heat
        self._hm_intensity     = 0.5        # deposit strength scale
        # World-space heatmap grid: resolution = cells per foot
        self._hm_cpf           = 4.0        # cells per foot (fixed resolution)
        self._hm_gw            = 1
        self._hm_gh            = 1
        # world bbox of the grid (feet)
        self._hm_wx0           = 0.0
        self._hm_wy0           = 0.0
        self._hm_wx1           = 10.0
        self._hm_wy1           = 10.0
        self._hm_grid          = np.full((1,1), self._hm_base, dtype=np.float32)
        self._hm_rgb           = np.zeros((1,1,3), dtype=np.uint8)
        self._hm_qimage        = QImage()

        self.fake_tags: list = []
        self._fake_tag_counter = 0
    ...
    def paintEvent(self, e):
        ...
        # ── heatmap background ────────────────────────────────────────────────
        if self.heatmap_enabled and not self._hm_qimage.isNull():
            # Map the world-space grid bbox to screen coordinates and stretch-blit
            s0 = self.to_screen(self._hm_wx0, self._hm_wy1)  # world top-left → screen
            s1 = self.to_screen(self._hm_wx1, self._hm_wy0)  # world bot-right → screen
            dst = QRect(int(s0.x()), int(s0.y()),
                        max(1, int(s1.x()-s0.x())),
                        max(1, int(s1.y()-s0.y())))
            src = QRect(0, 0, self._hm_qimage.width(), self._hm_qimage.height())
            p.drawImage(dst, self._hm_qimage, src)
        ...
        draw_map_scene(
            ...
            heatmap_active=self.heatmap_enabled,
        )

        # ── sprite size: proportional to room area ────────────────────────────
        # Target: person sprite ~= 8% of the room's "side length" in screen px
        world_hull = _convex_hull_world(self.anchors)
        area_ft2   = _poly_area(world_hull) if len(world_hull) >= 3 else 100.0
        side_ft    = math.sqrt(max(area_ft2, 1.0))
        sprite_px  = max(24, min(int(side_ft * self.scale * 0.10), 120))

        frames = get_walk_frames() if self.walk_anim_enabled else None
        tag_r  = max(7, min(int(self.scale * 0.22), 14))
        lsz    = max(8, min(11, int(self.scale * 0.14)))

        # ── fake tags (world-space → screen at draw time) ─────────────────────
        for ft in self.fake_tags:
            sp  = self.to_screen(ft.wx, ft.wy)
            col = QColor("#f0e040")
            if self.walk_anim_enabled and frames:
                sprite = frames[ft.current_frame_name()]
                sc_spr = sprite.scaled(sprite_px, sprite_px,
                                       Qt.AspectRatioMode.KeepAspectRatio,
                                       Qt.TransformationMode.SmoothTransformation)
                p.save()
                p.translate(sp)
                p.rotate(math.degrees(ft.angle))
                p.drawPixmap(-sc_spr.width()//2, -sc_spr.height()//2, sc_spr)
                p.restore()
            else:
                p.setPen(QPen(QColor("white"), 1.5)); p.setBrush(QBrush(col))
                p.drawEllipse(sp, tag_r, tag_r)
            p.setFont(QFont("Segoe UI", max(7, lsz-1), QFont.Weight.Bold))
            p.setPen(col)
            p.drawText(int(sp.x()) + tag_r + 3, int(sp.y()) - 2, ft.tag_id)

        # ── walking animation on real tags ────────────────────────────────────
        if self.walk_anim_enabled and frames:
            all_real = {}
            if self.tag_cal:
                for tid, pos in self.tag_cal.items():
                    if pos: all_real[tid] = pos
            if self.tag_raw:
                for tid, pos in self.tag_raw.items():
                    if tid not in all_real and pos: all_real[tid] = pos
            for tid, pos in all_real.items():
                sp    = self.to_screen(pos[0], pos[1])
                angle = getattr(self, '_real_tag_angles', {}).get(tid, 0.0)
                fname = getattr(self, '_real_tag_frames', {}).get(tid, "stand")
                sprite = frames[fname]
                sc_spr = sprite.scaled(sprite_px, sprite_px,
                                       Qt.AspectRatioMode.KeepAspectRatio,
                                       Qt.TransformationMode.SmoothTransformation)
    ...
    # ── Heatmap (world-space grid) ────────────────────────────────────────────
    def _hm_setup_grid(self):
        """Rebuild the world-space grid based on current anchor bounds + padding."""
        if not self.anchors: return
        xs = [v[0] for v in self.anchors.values()]
        ys = [v[1] for v in self.anchors.values()]
        pad = max(2.0, self._hm_exposure_ft * 0.5)
        self._hm_wx0 = min(xs) - pad; self._hm_wx1 = max(xs) + pad
        self._hm_wy0 = min(ys) - pad; self._hm_wy1 = max(ys) + pad
        gw = max(1, int((self._hm_wx1 - self._hm_wx0) * self._hm_cpf))
        gh = max(1, int((self._hm_wy1 - self._hm_wy0) * self._hm_cpf))
        if gw != self._hm_gw or gh != self._hm_gh:
            self._hm_gw   = gw; self._hm_gh   = gh
            self._hm_grid = np.full((gh, gw), self._hm_base, dtype=np.float32)
            self._hm_rgb  = np.zeros((gh, gw, 3), dtype=np.uint8)

    def _hm_world_to_cell(self, wx, wy):
        """Convert world-space (ft) to grid cell (float)."""
        cx = (wx - self._hm_wx0) * self._hm_cpf
        cy = (wy - self._hm_wy0) * self._hm_cpf   # y increases downward in grid
        return cx, cy

    def _hm_rebuild_image(self):
        scaled = np.clip(self._hm_grid * 255.0, 0, 255).astype(np.uint8)
        # flip vertically: world Y+ = up, but image row 0 = top
        flipped = scaled[::-1, :]
        self._hm_rgb[:, :, :] = HEATMAP_LUT[flipped]
        h, w, _ = self._hm_rgb.shape
        data = bytes(self._hm_rgb.tobytes())
        self._hm_qimage = QImage(data, w, h, 3 * w, QImage.Format.Format_RGB888)

    def _hm_deposit(self, wx1, wy1, wx2, wy2):
        """Deposit heat between two world-space positions (feet)."""
        dist_ft = math.hypot(wx2-wx1, wy2-wy1)
        if dist_ft >= self._hm_exposure_ft: return

        closeness = max(0.0, min(1.0, 1.0 - dist_ft / max(self._hm_exposure_ft, 1e-6)))
        strength  = (0.0012 + self._hm_intensity * 0.008) * (closeness ** 2.0)
        if strength <= 0: return

        # Convert to grid-cell space
        cx1, cy1 = self._hm_world_to_cell(wx1, wy1)
        cx2, cy2 = self._hm_world_to_cell(wx2, wy2)

        seg_dx = cx2 - cx1; seg_dy = cy2 - cy1
        seg_len_sq = seg_dx**2 + seg_dy**2

        # Influence radius in cells (exposure_ft converted to cells)
        inf_r = max(2.0, self._hm_exposure_ft * self._hm_cpf * 0.5)
        pad   = int(math.ceil(inf_r))

        mn_x = int(max(0, math.floor(min(cx1, cx2) - pad)))
        mx_x = int(min(self._hm_gw - 1, math.ceil(max(cx1, cx2) + pad)))
        mn_y = int(max(0, math.floor(min(cy1, cy2) - pad)))
        mx_y = int(min(self._hm_gh - 1, math.ceil(max(cy1, cy2) + pad)))
        if mn_x > mx_x or mn_y > mx_y: return

        xs = np.arange(mn_x, mx_x + 1, dtype=np.float32) + 0.5
        ys = np.arange(mn_y, mx_y + 1, dtype=np.float32) + 0.5
        xx, yy = np.meshgrid(xs, ys)

        if seg_len_sq < 1e-6:
            d = np.sqrt((xx - cx1)**2 + (yy - cy1)**2)
        else:
            t = ((xx - cx1)*seg_dx + (yy - cy1)*seg_dy) / seg_len_sq
            t = np.clip(t, 0.0, 1.0)
            px2 = cx1 + t*seg_dx; py2 = cy1 + t*seg_dy
            d = np.sqrt((xx - px2)**2 + (yy - py2)**2)

        sigma = max(1.0, inf_r * (0.55 - 0.20 * closeness))
        field = np.exp(-(d**2) / (2.0 * sigma**2)).astype(np.float32)

        mid_cx = (cx1+cx2)*0.5; mid_cy = (cy1+cy2)*0.5
        md  = np.sqrt((xx - mid_cx)**2 + (yy - mid_cy)**2)
        mw  = np.exp(-(md**2) / (2.0 * max(2.0, inf_r*0.95)**2)).astype(np.float32)

        self._hm_grid[mn_y:mx_y+1, mn_x:mx_x+1] += field * (0.65 + 0.35 * mw) * strength

    def _hm_decay_step(self):
        self._hm_grid += (self._hm_base - self._hm_grid) * (1.0 - self._hm_decay)
        np.clip(self._hm_grid, self._hm_base, 1.0, out=self._hm_grid)

    def heatmap_tick(self, all_world_positions):
        """Called every RTLS loop. all_world_positions: list of (wx,wy) in feet."""
        if not self.heatmap_enabled: return
        self._hm_setup_grid()
        self._hm_decay_step()
        n = len(all_world_positions)
        for i in range(n):
            for j in range(i + 1, n):
                wx1, wy1 = all_world_positions[i]
                wx2, wy2 = all_world_positions[j]
                self._hm_deposit(wx1, wy1, wx2, wy2)
        np.clip(self._hm_grid, self._hm_base, 1.0, out=self._hm_grid)
        self._hm_rebuild_image()

    def reset_heatmap(self):
        self._hm_grid.fill(self._hm_base)
        self._hm_rebuild_image()
        self.update()

    # ── Fake tag management ───────────────────────────────────────────────────
    def add_fake_tag(self):
        """Place a fake tag at a random position strictly inside the anchor polygon (world-space)."""
        if len(self.anchors) < 3: return
        world_poly = _convex_hull_world(self.anchors)
        if len(world_poly) < 3: return
        cx = sum(p[0] for p in world_poly) / len(world_poly)
        cy = sum(p[1] for p in world_poly) / len(world_poly)
        xs = [p[0] for p in world_poly]; ys = [p[1] for p in world_poly]
        span_x = max(xs) - min(xs); span_y = max(ys) - min(ys)
        for _ in range(300):
            rx = cx + random.uniform(-span_x*0.45, span_x*0.45)
            ry = cy + random.uniform(-span_y*0.45, span_y*0.45)
            if _point_in_poly(rx, ry, world_poly):
                self._fake_tag_counter += 1
                self.fake_tags.append(FakeTag(f"F{self._fake_tag_counter}", rx, ry))
                self.update()
                return
        # fallback: centroid
        self._fake_tag_counter += 1
        self.fake_tags.append(FakeTag(f"F{self._fake_tag_counter}", cx, cy))
        self.update()

    def clear_fake_tags(self):
        self.fake_tags.clear()
        self.update()

    # ── Walking animation tick ─────────────────────────────────────────────────
    def _walk_tick(self):
        if not self.walk_anim_enabled and not self.fake_tags: return
        # World-space polygon for boundary enforcement
        world_poly = _convex_hull_world(self.anchors) if self.anchors else []

        # Advance fake tags in world-space
        for ft in self.fake_tags:
            ft.update(world_poly)

        # Advance real-tag walking animation state
        if self.walk_anim_enabled:
            if not hasattr(self, '_real_tag_walkers'):
                self._real_tag_walkers = {}
                self._real_tag_angles  = {}
                self._real_tag_frames  = {}
            for tid in ["T0", "T1", "T2"]:
                pos = self.tag_cal.get(tid) or self.tag_raw.get(tid)
                if pos is None:
                    self._real_tag_walkers.pop(tid, None)
                    continue
                wx, wy = pos[0], pos[1]
                if tid not in self._real_tag_walkers:
                    self._real_tag_walkers[tid] = {
                        "is_walking": False, "walk_phase": 0,
                        "anim_elapsed": 0.0, "prev_wx": wx, "prev_wy": wy,
                        "angle": 0.0, "dir_samples": []
                    }
                w = self._real_tag_walkers[tid]
                dx = wx - w["prev_wx"]; dy = wy - w["prev_wy"]
                speed_ft = math.hypot(dx, dy)   # ft per tick
                w["prev_wx"] = wx; w["prev_wy"] = wy

                if speed_ft > 1e-4:
                    w["dir_samples"].append((dx, dy))
                    if len(w["dir_samples"]) > DIR_HISTORY: w["dir_samples"].pop(0)
                    ma = _mean_angle(w["dir_samples"])
                    if ma is not None: w["angle"] = _smooth_angle(w["angle"], ma, DIR_SMOOTHING)

                # Use px speed proxy for animation timing (scale ft → px)
                spd_px = speed_ft * self.scale
                ft_time = _speed_to_frame_time(max(spd_px, STOP_ANIM_SPEED + 1e-6))
                if spd_px > STOP_ANIM_SPEED:
                    w["is_walking"] = True
                    w["anim_elapsed"] += WALK_TICK_MS / 1000.0
                    if ft_time:
                        while w["anim_elapsed"] >= ft_time:
                            w["anim_elapsed"] -= ft_time
                            w["walk_phase"] = 1 - w["walk_phase"]
                else:
                    w["is_walking"] = False; w["walk_phase"] = 0; w["anim_elapsed"] = 0.0

                self._real_tag_angles[tid] = w["angle"]
                self._real_tag_frames[tid] = (
                    "left" if w["walk_phase"] == 0 else "right"
                ) if w["is_walking"] else "stand"

        if self.fake_tags or self.walk_anim_enabled:
            self.update()
...
class RightPanel(QFrame):
    def __init__(self):
        ...
        # ── THEMES ─────────────────────────────────────────────────────────
        th_box = section_box()
        thi = QVBoxLayout(th_box); thi.setContentsMargins(10,8,10,8); thi.setSpacing(6)
        thi.addWidget(section_label("THEMES")); thi.addWidget(h_line())

        # Heat map on/off
        hm_row = QHBoxLayout(); hm_row.setSpacing(8)
        hm_lbl = QLabel("Heat Map"); hm_lbl.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:10px;font-weight:600;")
        self._hm_toggle = QPushButton("OFF"); self._hm_toggle.setCheckable(True)
        self._hm_toggle.setFixedSize(QSize(44,22))
        self._hm_toggle.setStyleSheet(
            f"QPushButton{{background:{SURFACE};color:{TEXT_SECONDARY};border:1px solid {BORDER};"
            f"border-radius:3px;font-size:10px;font-weight:700;}}"
            f"QPushButton:checked{{background:{ACCENT};color:#000;border:1px solid {ACCENT};}}")
        self._hm_toggle.toggled.connect(self._on_hm_toggle)
        hm_row.addWidget(hm_lbl); hm_row.addStretch(); hm_row.addWidget(self._hm_toggle)
        thi.addLayout(hm_row)

        # Heat map reset
        hm_reset = QPushButton("Reset Heat"); hm_reset.setObjectName("paramBtn")
        hm_reset.clicked.connect(self._on_hm_reset)
        thi.addWidget(hm_reset)

        thi.addWidget(h_line())

        # Exposure distance (ft)
        def _th_slider_row(parent_lay, label, init_val, lo, hi, step, fmt, cb):
            row2 = QHBoxLayout(); row2.setSpacing(6)
            lbl2 = QLabel(label); lbl2.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:9px;"); lbl2.setFixedWidth(160)
            val2 = QLabel(fmt.format(init_val))
            val2.setStyleSheet(f"color:{TEXT_PRIMARY};font-size:9px;font-weight:600;font-family:Consolas;"); val2.setFixedWidth(40)
            row2.addWidget(lbl2); row2.addWidget(val2); parent_lay.addLayout(row2)
            sl2 = QSlider(Qt.Orientation.Horizontal)
            sl2.setRange(int(lo/step), int(hi/step)); sl2.setValue(int(init_val/step))
            sl2.setStyleSheet(
                f"QSlider::groove:horizontal{{background:{SURFACE};height:4px;border-radius:2px;}}"
                f"QSlider::handle:horizontal{{background:{ACCENT};width:12px;height:12px;margin:-4px 0;border-radius:6px;}}"
                f"QSlider::sub-page:horizontal{{background:{ACCENT};height:4px;border-radius:2px;}}")
            def _on_change(v, vl=val2, f=fmt, s=step, callback=cb):
                val_real = v*s; vl.setText(f.format(val_real)); callback(val_real)
            sl2.valueChanged.connect(_on_change)
            parent_lay.addWidget(sl2)
            return sl2

        _th_slider_row(thi, "Exposure distance (ft):", 8.0, 1.0, 40.0, 0.5, "{:.1f}", self._on_hm_exposure)
        _th_slider_row(thi, "Intensity (0=soft, 1=hot):", 0.5, 0.0, 1.0, 0.01, "{:.2f}", self._on_hm_intensity)
        _th_slider_row(thi, "Heat linger (0=fast, 1=slow):", 0.993, 0.97, 0.9999, 0.0001, "{:.4f}", self._on_hm_decay)

        thi.addWidget(h_line())

        # Place fake tag
        fake_row = QHBoxLayout(); fake_row.setSpacing(8)
        fake_lbl = QLabel("Fake Dot (test)"); fake_lbl.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:10px;font-weight:600;")
        add_fake_btn = QPushButton("+ Add"); add_fake_btn.setObjectName("paramBtn")
        add_fake_btn.setFixedWidth(54)
        add_fake_btn.clicked.connect(self._on_add_fake_tag)
        clr_fake_btn = QPushButton("Clear"); clr_fake_btn.setObjectName("deleteBtn")
        clr_fake_btn.setFixedWidth(50)
        clr_fake_btn.clicked.connect(self._on_clear_fake_tags)
        fake_row.addWidget(fake_lbl); fake_row.addStretch(); fake_row.addWidget(add_fake_btn); fake_row.addWidget(clr_fake_btn)
        thi.addLayout(fake_row)

        thi.addWidget(h_line())

        # Walking animation on/off
        walk_row = QHBoxLayout(); walk_row.setSpacing(8)
        walk_lbl = QLabel("Walking Animation"); walk_lbl.setStyleSheet(f"color:{TEXT_SECONDARY};font-size:10px;font-weight:600;")
        self._walk_toggle = QPushButton("OFF"); self._walk_toggle.setCheckable(True)
        self._walk_toggle.setFixedSize(QSize(44,22))
        self._walk_toggle.setStyleSheet(
            f"QPushButton{{background:{SURFACE};color:{TEXT_SECONDARY};border:1px solid {BORDER};"
            f"border-radius:3px;font-size:10px;font-weight:700;}}"
            f"QPushButton:checked{{background:{ACCENT};color:#000;border:1px solid {ACCENT};}}")
        self._walk_toggle.toggled.connect(self._on_walk_toggle)
        walk_row.addWidget(walk_lbl); walk_row.addStretch(); walk_row.addWidget(self._walk_toggle)
        thi.addLayout(walk_row)

        self.lay.addWidget(th_box)
        self.lay.addStretch()
    ...
    def _on_hm_toggle(self, checked):
        self._hm_toggle.setText("ON" if checked else "OFF")
        c = self._get_canvas()
        if c:
            c.heatmap_enabled = checked
            if not checked: c.reset_heatmap()
            else:           c._hm_setup_grid(); c._hm_rebuild_image(); c.update()

    def _on_hm_reset(self):
        c = self._get_canvas()
        if c: c.reset_heatmap()

    def _on_hm_exposure(self, val):
        c = self._get_canvas()
        if c: c._hm_exposure_ft = val

    def _on_hm_intensity(self, val):
        c = self._get_canvas()
        if c: c._hm_intensity = val

    def _on_hm_decay(self, val):
        c = self._get_canvas()
        if c: c._hm_decay = val

    def _on_add_fake_tag(self):
        c = self._get_canvas()
        if c: c.add_fake_tag()

    def _on_clear_fake_tags(self):
        c = self._get_canvas()
        if c: c.clear_fake_tags()

    def _on_walk_toggle(self, checked):
        self._walk_toggle.setText("ON" if checked else "OFF")
        c = self._get_canvas()
        if c:
            c.walk_anim_enabled = checked
            if not checked:
                # reset animation state
                if hasattr(c, '_real_tag_walkers'): c._real_tag_walkers.clear()
                if hasattr(c, '_real_tag_angles'):  c._real_tag_angles.clear()
                if hasattr(c, '_real_tag_frames'):  c._real_tag_frames.clear()
...
class MainWindow(QMainWindow):
    ...
    def _rtls_loop(self):
        ...
        # ── Heatmap tick ───────────────────────────────────────────────────────
        if canvas.heatmap_enabled:
            all_world = []
            # real tags — already in world-space (feet)
            for t_id in ["T0", "T1", "T2"]:
                pos = canvas.tag_cal.get(t_id) or canvas.tag_raw.get(t_id)
                if pos:
                    all_world.append((pos[0], pos[1]))
            # fake tags — wx,wy are world-space (feet)
            for ft in canvas.fake_tags:
                all_world.append((ft.wx, ft.wy))
            canvas.heatmap_tick(all_world)
