...
def _normalize_stroke_color(value: str | None) -> str:
    ...
    if stroke.startswith("#"):
        ...
        return stroke
    rgb_match = re.match(r"rgb\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)\s*\)", stroke)
    if rgb_match:
        r, g, b = (max(0, min(255, int(group))) for group in rgb_match.groups())
...
def extract_styled_segments_from_svg(filepath: str):
    """
    Parse an SVG file and extract line-like geometry with color/style metadata.
    Returns a tuple (entries, error_msg) where each entry is:
    ...
    if has_cad_meta:
        try:
            ...
            def reverse_x(tx):
                return (tx - c_ox) / c_scale + c_mx

            def reverse_y(ty):
                return (doc_size - ty - c_oy) / c_scale + c_my
        except ValueError:
            has_cad_meta = False
...
def extract_lines_from_svg(filepath: str):
    ...
    return [entry["segment"] for entry in entries], error
